<#
.SYNOPSIS
  Deploy Career Mentor → ChatGPT GPTs 格式（平鋪 10 個文件）
  保護：GPTs_DEPLOY-GUIDE.md
  清除：output/（完整重建）
#>
...
Write-Host "  Deploy Target: ChatGPT GPTs" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════" -ForegroundColor DarkCyan
...
Write-Host ""
Write-Host "✅  ChatGPT GPTs 部署完成" -ForegroundColor Green
Write-Host "    輸出路徑：$outputDir"
Write-Host ""
Write-Host "部署提示：將 output/instructions.md 貼入 GPT Instructions，其餘 9 個文件上傳至 Knowledge" -ForegroundColor DarkGray
Write-Host ""
